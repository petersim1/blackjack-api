...
class Game:
    ...
    shrink_deck: bool = (
        True  # if False, will randomly select cards uniformly, and deck won't run out.
    )
    n_decks: int = 6
    ...
    def deal_init(self) -> None:
        ...
        if self.house.cards[0].total == 21:
            # The flow is -> dealer shows a card, if it's an Ace or 10, then ask players if they want insurance. # noqa: E501
            # Dealer will then check the card, if it's blackjack, the round is over. If it's not blackjack, surrender is allowed. # noqa: E501
            # Since I'm doing this without insurance, it isn't relevant. End hand immediately if blackjack. # noqa: E501
