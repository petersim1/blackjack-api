...
from app.modules.cards import Card, Cards
...
class Player:
    ...
    def _split(self, cards: List[Card]) -> None:
        ...
        # Calling this add_cards() fct will inherently create an updated "total" variable. # noqa: E501
        # if we create the new Cards in the .insert() method, we won't have the "total" variable yet. # noqa: E501
    ...
    def get_value(self) -> Tuple[int, bool]:
        """
        gets the card total and whether there's a useable ace
        for the current hand of cards for a player
        """
    ...
    def get_valid_moves(self) -> List[str]:
        ...
        if (
            i_hand < 0
        ):  # catches when the player is done. Don't need to additional check .is_done()
        ...
        can_double = (
            (n == 2)
            and (((n_hands > 1) & self.rules.double_after_split) or (n_hands == 1))
            and can_hit
        )
